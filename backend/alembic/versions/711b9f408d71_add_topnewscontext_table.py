...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "source_country", sqlmodel.sql.sqltypes.AutoString(length=2), nullable=False
        ),
        sa.Column(
            "language", sqlmodel.sql.sqltypes.AutoString(length=2), nullable=False
        ),
    ...
    op.create_index(
        op.f("ix_topnewscontext_chat_id"), "topnewscontext", ["chat_id"], unique=True
    )
