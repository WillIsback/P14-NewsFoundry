...
async function readJsonResponse(
	...
	try {
		...
		const err =
			error instanceof Error ? error : new Error("Invalid JSON payload");
...
function validateRequestPayload<TReq>(
	...
	| { ok: true; payload: TReq | undefined }
...
function computeRetryDelay(attempt: number, retry: RetryOptions): number {
	...
	const exponential = Math.min(maxDelayMs, initialDelayMs * 2 ** (attempt - 1));
