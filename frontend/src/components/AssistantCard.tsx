	...
	<>
			{variant === "welcome" ? (
				...
				<div className="flex flex-col w-full h-fit gap-8">
					...
					? 
						(
								messages?.map((message) => (
									<div key={message.id} className={`flex gap-2.5 ${message.type === "user" ? "justify-end" : "justify-start"}`}>
					...
					: 
						(
							<p className="text-slate-800 text-center">Aucun message pour le moment. Commencez la conversation en envoyant un message !</p>
						)
					}	
				</div>
			)}
